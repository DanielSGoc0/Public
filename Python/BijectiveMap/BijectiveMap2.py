...
INT_ORDER = 5
SPLINE_ORDER = 9
OFFSET = 2**INT_ORDER + (N + 2)*(SPLINE_ORDER + 2)
# OFFSET = 100
h = T / 2**INT_ORDER
original_range = range(-OFFSET, OFFSET + 1)
...
def solve_ODE(f, y0):
	...
	sol1 = scipy.integrate.solve_ivp(f, (0, OFFSET * h), y0, method='DOP853', t_eval=nodes[OFFSET:], first_step=h, max_step=h)
	sol2 = scipy.integrate.solve_ivp(lambda t, z: neg(f(-t, z)), (0, OFFSET * h), y0, method='DOP853', t_eval=nodes[OFFSET + 1:], first_step=h, max_step=h)
...
def get_V_0T(i, j):
	...
	y = [np.exp(2*L_0_[i][j](n) - L2__[j](n)) for n in nodes[OFFSET:OFFSET + 2**INT_ORDER + 1]]
...
def calc():
	...
	# print(L2_T[N])
	# return L2_T[N]
	# ANS = L2_T[N]/(2*logcosh(T) + L2_0[N] - 2*L_0_[N][0](-T))
	ANS = 2.0/(1.0/L2_T[N] + 1.0/(2*logcosh(T) + L2_0[N] - 2*L_0_[N][0](-T)))
	print(L2_T[N], 2*logcosh(T) + L2_0[N] - 2*L_0_[N][0](-T))
	print(ANS)
	return ANS
