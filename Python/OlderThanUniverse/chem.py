...
def value_generator(x):
	global generator
	return (generator[0] * x*x + generator[1])/(x*x + generator[2]*x + generator[3])
...
def value_optimum(x):
	global optimum
	return (optimum[0] * x * x + optimum[1])/(x*x + optimum[2] * x + optimum[3])
...
def value_present(x):
	global present
	return (present[0] * x * x + present[1])/(x*x + present[2] * x + present[3])
...
def da():
	...
	DA = 0.0
	for i in range(0, N):
		DA += px[i]*px[i]/(px[i]*px[i] + present[2] * px[i] + present[3]) * (value_present(px[i]) - py[i])
	return 2*DA
...
def db():
	...
	DB = 0.0
	for i in range(0, N):
		DB += 1.0/(px[i]*px[i] + present[2] * px[i] + present[3]) * (value_present(px[i]) - py[i])
	return 2*DB
...
def dc():
	...
	DC = 0.0
	for i in range(0, N):
		DC -= px[i] * (present[0] * px[i]*px[i] + present[1])/((px[i]*px[i] + present[2] * px[i] + present[3])**2) * (value_present(px[i]) - py[i])
	return 2*DC
...
def dd():
	...
	DD = 0.0
	for i in range(0, N):
		DD -= (present[0] * px[i]*px[i] + present[1])/((px[i]*px[i] + present[2] * px[i] + present[3])**2) * (value_present(px[i]) - py[i])
	return 2*DD
