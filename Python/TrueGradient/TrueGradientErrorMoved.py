...
def calc(A, stats):
	...
	# DF is the true gradient
	# Sigma_ij = e^(-||Xi - Xj||^2 / 2)
	# coordinates from derivatives are: 0, 1, ... , N
	# coordinates from error are: N + 1, ... , 2*N
	X = np.zeros((N + 1, 2*N + 1), dtype='double')
	F = np.zeros((N + 1), dtype='double')
	DF = np.zeros((N + 1, 2*N + 1), dtype='double')
	Sigma = np.zeros((2*N + 1, 2*N + 1), dtype='double')
	...
	for n in range(1, N + 1):
		...
		# Firstly, we update X[n] and Sigma:
		for i in range(n):
			X[n] += A[n - 1][i] * DF[i]
			X[n][N + 1 + i] += A[n - 1][i] * err
		...
		# Next, calculate the magnitude^2 of derivative orthogonal vector.
		...
		M = np.zeros((n*(2*N + 1), n*(2*N + 1)), dtype='double')
		for a in range(n):
			for b in range(n):
				for i in range(2*N + 1):
					for j in range(2*N + 1):
						M[a*(2*N + 1) + i][b*(2*N + 1) + j] = Sigma[a][b] * (kronecker(i, j) - (X[a][i] - X[b][i]) * (X[a][j] - X[b][j]))

		d = np.zeros(n*(2*N + 1), dtype='double')
		for a in range(n):
			for i in range(2*N + 1):
				d[a*(2*N + 1) + i] = DF[a][i]
		...
		L = np.zeros((2*N + 1, n*(2*N + 1)), dtype='double')
		for b in range(n):
			for i in range(2*N + 1):
				for j in range(2*N + 1):
					L[i][b*(2*N + 1) + j] = Sigma[n][b] * (kronecker(i, j) - (X[n][i] - X[b][i]) * (X[n][j] - X[b][j]))

		mean = np.dot(L, r)


		# As well as the mean of F(Xn).
		l = np.zeros(n*(2*N + 1), dtype='double')
		for b in range(n):
			for j in range(2*N + 1):
				l[b*(2*N + 1) + j] = Sigma[n][b] * (X[n][j] - X[b][j])
