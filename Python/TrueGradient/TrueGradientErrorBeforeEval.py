...
# In this modification, the oracle returns f(x_{Alg} + err),
# where err is orthogonal and has fixed modulus and changer is some function.
# TBH I don't even know if this implementation works.
import scipy
import numpy as np
import math
import random
...
def calc(A, stats):
	...
	# DF describes the geometry of gradients dF(Xn + En) and is lower triangular
	# Sigma_ij = e^(-||Xi + Ei - Xj - Ej||^2 / 2)
	...
	for n in range(1, N + 1):
		...
		for i in range(n):
			Sigma[n][i] = math.exp(-np.dot(X[n] - X[i], X[n] - X[i]) / 2.0 - err2)
			Sigma[i][n] = Sigma[n][i]
		Sigma[n][n] = 1.0
